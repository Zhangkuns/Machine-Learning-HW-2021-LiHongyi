...
# Dataset
"""
- Original dataset is [Vox-celeb1](https://www.robots.ox.ac.uk/~vgg/data/voxceleb/).
- The [license](https://creativecommons.org/licenses/by/4.0/) and
- [complete version](https://www.robots.ox.ac.uk/~vgg/data/voxceleb/files/license.txt) of Vox-celeb1.
- We randomly select 600 speakers from Vox-celeb1.
...
def collate_batch(batch):
    ...
    mel, speaker = zip(*batch)  # 与 zip 相反，可理解为解压，为zip的逆过程，可用于矩阵的转置
...
def get_dataloader(data_dir, batch_size, n_workers):
    ...
    train_loader = DataLoader(
        ...
        # See Use pinned memory buffers for more details on when and how to use pinned memory generally.
        # passing pin_memory=True to a DataLoader will automatically put the fetched data Tensors in pinned memory,
...
- TransformerEncoderLayer:
  ...
  - Parameters:
    ...
    - n_head: the number of heads of the multiheadattention models (required).
...
def valid(dataloader, model, criterion, device):
    ...
    pbar = tqdm(total=len(dataloader.dataset), ncols=0, desc="Valid", unit=" uttr")  # 显示进度条
